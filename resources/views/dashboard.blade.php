<x-app-layout>
    ...
    <div style="margin-bottom:24px;">
        ...
        <div style="display:flex;align-items:center;gap:14px;padding:14px 18px;border-radius:12px;margin-bottom:8px;
            ...
            <div style="font-size:16px;flex-shrink:0;width:28px;height:28px;border-radius:50%;background:{{ $warn['over'] ? '#dc2626' : '#f59e0b' }};display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:13px;">
                {{ $warn['over'] ? '!' : '⚠' }}
            </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px;">
        <div class="card" style="background:#0f1a3a;border-color:#0f1a3a;">
            ...
            <div style="font-family:'Fraunces',serif;font-size:28px;color:#fff;letter-spacing:-1px;margin-bottom:8px;">
        ...
        <div class="card" style="border-top:3px solid #059669;">
            ...
            <div style="font-family:'Fraunces',serif;font-size:28px;color:#059669;letter-spacing:-1px;margin-bottom:8px;">
        ...
        <div class="card" style="border-top:3px solid #dc2626;">
            ...
            <div style="font-family:'Fraunces',serif;font-size:28px;color:#dc2626;letter-spacing:-1px;margin-bottom:8px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 340px;gap:16px;">
        ...
        {{-- Quick Stats + Budget + Wishlist Widget --}}
        <div style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div class="card">
                <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.5px;color:#8a96b0;font-weight:600;margin-bottom:12px;">Rasio Tabungan</div>
            ...
            {{-- Wishlist Widget --}}
            @if($wishlistWidget->isNotEmpty())
            <div class="card">
                <div class="card-header" style="margin-bottom:14px;">
                    <div class="card-title">◇ Wishlist</div>
                    <a href="{{ route('wishlists.index') }}" class="card-link">Lihat semua →</a>
                </div>
                @foreach($wishlistWidget as $item)
                <div style="margin-bottom:14px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;">
                        <span style="font-size:12px;font-weight:500;color:#374151;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:140px;">{{ $item->name }}</span>
                        <span style="font-size:11px;font-weight:600;color:{{ $item->status === 'ready' ? '#059669' : '#2e5fba' }};">
                            {{ $item->status === 'ready' ? 'Siap! 🎉' : $item->progress . '%' }}
                        </span>
                    </div>
                    <div style="height:5px;background:#f0f4fc;border-radius:4px;overflow:hidden;">
                        <div style="height:100%;width:{{ $item->progress }}%;border-radius:4px;background:{{ $item->status === 'ready' ? '#059669' : '#2e5fba' }};"></div>
                    </div>
                    <div style="font-size:10.5px;color:#a0aec0;margin-top:3px;">
                        Rp {{ number_format($item->saved_amount, 0, ',', '.') }} / Rp {{ number_format($item->target_price, 0, ',', '.') }}
                    </div>
                </div>
                @endforeach
            </div>
